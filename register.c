...
//    if (-1 == is_allow_to_write(reg_addr))
//    {
//        //DEBUG_ERROR("the reg address[%d] is not allowed to be updated since no permission\r\n", reg_addr);
//        reg_modbus_exception_response(REG_MODBUS_OPR_SINGLE_WRITE, _errcode_INVALID_DATA_ADDRESS);
//        return -1;
//    }
    
    memset(__local_reg_data, 0, sizeof(__local_reg_data));
    __local_reg_data[0] = ccts(buf[4], buf[5]);
    reg_write(reg_addr, __local_reg_data, 1);

       
    reg_modbus_single_write_command_response();
...
u8 reg_modbus_single_write_command_response()
{
    reg_modbus_send_response_packet(reg_context.packet_data, 8);
